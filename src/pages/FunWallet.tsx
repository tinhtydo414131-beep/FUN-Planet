...
const ERC20_ABI = ["function balanceOf(address owner) view returns (uint256)", "function transfer(address to, uint256 amount) returns (bool)", "function decimals() view returns (uint8)", "function approve(address spender, uint256 amount) returns (bool)", "function allowance(address owner, address spender) view returns (uint256)"];
export default function FunWallet() {
  ...
  useEffect(() => {
    if (!user?.id) return;
    const channel = supabase.channel('wallet_transactions_incoming').on('postgres_changes', {
      event: 'INSERT',
      schema: 'public',
  ...
  const getCamlyBalance = async (address: string) => {
    try {
      ...
      setTokenBalances(prev => ({
        ...prev,
        CAMLY: parseFloat(formatted).toFixed(2)
      }));
    } catch (error) {
  ...
  const getUSDTBalance = async (address: string) => {
    try {
      ...
      setTokenBalances(prev => ({
        ...prev,
        USDT: parseFloat(formatted).toFixed(2)
      }));
    } catch (error) {
  ...
  const fetchTokenPrices = async () => {
    ...
    try {
      ...
      const mainResponse = await fetch('https://api.coingecko.com/api/v3/simple/price?ids=binancecoin,ethereum,tether,funtoken&vs_currencies=usd&include_24hr_change=true');

      // Fetch CAMLY separately
      const camlyResponse = await fetch('https://api.coingecko.com/api/v3/simple/price?ids=camly-coin&vs_currencies=usd&include_24hr_change=true');
  ...
  const fetchTransactionHistory = async () => {
    ...
    try {
      const {
        data,
        error
      } = await supabase.from("wallet_transactions").select("*").or(`from_user_id.eq.${user.id},to_user_id.eq.${user.id}`).not("transaction_hash", "is", null).order("created_at", {
        ascending: false
      }).limit(50);
      if (error) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const {
        ...
        error: lookupError
      } = await supabase.from("profiles").select("id, username, wallet_address").ilike("wallet_address", normalizedRecipient).maybeSingle();
  ...
  const validateAddresses = () => {
    ...
    const rawAddresses = bulkAddresses.split('\n').map(addr => addr.trim()).filter(addr => addr.length > 0); // Remove empty lines
  ...
  const handleBulkSend = async () => {
    ...
    try {
      ...
      try {
        ...
        const multiSendTx = await multisendContract.multiSendToken(camlyToken.contract, addresses, amounts, {
          gasLimit
        });
      ...
      if (!txHash) {
        ...
        } = await supabase.from('profiles').select('id, wallet_address').in('wallet_address', addresses.map(addr => addr.toLowerCase()));
        console.log('Found recipient profiles:', recipientProfiles);

        // Create a map of address to user_id
        const addressToUserId = new Map(recipientProfiles?.map(p => [p.wallet_address?.toLowerCase(), p.id]) || []);
        ...
        const transactionRecords = addresses.map(recipientAddress => {
          ...
          return {
            ...
            // Set to_user_id if found, null otherwise
            amount: amount,
  ...
  return <div className="min-h-screen bg-white relative overflow-hidden pb-20">
      <div className="container mx-auto px-3 sm:px-4 py-4 sm:py-8 max-w-4xl relative z-10">
        ...
        opacity: 1
          ...
          <div className="flex items-center justify-center gap-3">
            ...
            scale: 0.95
          }} className="relative w-14 h-14 rounded-full font-bold flex items-center justify-center transition-all duration-300 border-0 overflow-hidden group" style={{
            ...
            boxShadow: '0 0 30px rgba(6, 182, 212, 0.5), 0 0 60px rgba(139, 92, 246, 0.3), 0 8px 32px rgba(236, 72, 153, 0.4)'
              ...
              <div className="absolute inset-0 opacity-60">
                <div className="absolute top-0 left-0 w-1/2 h-1/2 bg-gradient-to-br from-white/40 via-cyan-200/20 to-transparent" style={{
                clipPath: 'polygon(0 0, 100% 0, 50% 100%, 0 50%)'
              }} />
                <div className="absolute top-0 right-0 w-1/2 h-1/2 bg-gradient-to-bl from-white/30 via-purple-200/20 to-transparent" style={{
                clipPath: 'polygon(0 0, 100% 0, 100% 50%, 50% 100%)'
              }} />
                <div className="absolute bottom-0 left-0 w-1/2 h-1/2 bg-gradient-to-tr from-pink-300/30 via-transparent to-transparent" style={{
                clipPath: 'polygon(0 100%, 50% 0, 0 50%)'
              }} />
                <div className="absolute bottom-0 right-0 w-1/2 h-1/2 bg-gradient-to-tl from-violet-300/30 via-transparent to-transparent" style={{
              ...
              <div className="absolute bottom-3 right-2 w-1 h-1 bg-cyan-200 rounded-full blur-[1px] animate-pulse opacity-70" style={{
              animationDelay: '0.5s'
            }} />
              {/* Rainbow border glow */}
            ...
            scale: 0.95
          }} className="relative flex-1 max-w-md h-14 rounded-[28px] font-bold flex items-center justify-center gap-3 transition-all duration-300 border-0 text-white text-lg overflow-hidden group" style={{
            background: 'linear-gradient(135deg, #06b6d4 0%, #8B5CF6 50%, #ec4899 100%)',
            boxShadow: '0 0 30px rgba(6, 182, 212, 0.5), 0 0 60px rgba(139, 92, 246, 0.3), 0 8px 32px rgba(236, 72, 153, 0.4)'
          }}>
              {/* Diamond facet overlay */}
              <div className="absolute inset-0 opacity-50">
                ...
                <div className="absolute top-0 left-1/4 w-1/2 h-1/2 bg-gradient-to-b from-white/25 via-purple-200/15 to-transparent" style={{
                clipPath: 'polygon(50% 0, 100% 100%, 0 100%)'
              }} />
              </div>
              {/* Prismatic light lines */}
              <div className="absolute inset-0 opacity-40 overflow-hidden">
                <div className="absolute top-0 left-1/4 w-0.5 h-full bg-gradient-to-b from-transparent via-cyan-300 to-transparent" style={{
                transform: 'rotate(15deg)',
                transformOrigin: 'top'
              }} />
                <div className="absolute top-0 right-1/4 w-0.5 h-full bg-gradient-to-b from-transparent via-purple-300 to-transparent" style={{
                transform: 'rotate(-15deg)',
                transformOrigin: 'top'
              }} />
              ...
              <div className="absolute top-4 right-8 w-1.5 h-1.5 bg-cyan-200 rounded-full blur-[1px] animate-pulse opacity-70" style={{
              animationDelay: '0.3s'
            }} />
              <div className="absolute bottom-3 left-1/3 w-1 h-1 bg-purple-200 rounded-full blur-[1px] animate-pulse opacity-60" style={{
              animationDelay: '0.7s'
            }} />
              <div className="absolute bottom-4 right-1/4 w-1.5 h-1.5 bg-pink-200 rounded-full blur-[1px] animate-pulse opacity-75" style={{
              animationDelay: '1s'
            }} />
              {/* Rainbow border glow */}
        ...
        scale: 1
          ...
          duration: 2,
          repeat: Infinity
        }} className="w-32 h-32 sm:w-40 sm:h-40 mx-auto mb-6 sm:mb-8 rounded-full flex items-center justify-center shadow-2xl relative overflow-hidden" style={{
          ...
          boxShadow: '0 0 80px rgba(157,0,255,0.8)'
            ...
            <Button onClick={connectWallet} className="text-xl sm:text-2xl font-black px-8 sm:px-12 py-6 sm:py-8 h-auto relative overflow-hidden group border-0 w-full sm:w-auto" style={{
              ...
              <Card className="gradient-border rounded-2xl overflow-hidden relative bg-white backdrop-blur-sm shadow-2xl">
                <CardContent className="p-4 sm:p-6 md:p-8">
                  ...
                  <div className="flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-0 mb-4 sm:mb-6">
                    ...
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <Button variant="outline" className="font-bold text-base sm:text-lg px-4 sm:px-6 py-4 sm:py-6 h-auto w-full sm:w-auto">
                      ...
                      <DropdownMenuContent className="border border-border bg-card/95 backdrop-blur-lg z-[100] w-48 sm:w-auto">
                        {networks.map(network => <DropdownMenuItem key={network.id} onClick={() => switchNetwork(network)} className="px-3 sm:px-4 py-2 sm:py-3 cursor-pointer">
                  ...
                  <div className="flex items-center justify-center gap-2 mb-3 sm:mb-4">
                    ...
                    <Button variant="ghost" size="sm" onClick={copyAddress} className="h-7 w-7 sm:h-6 sm:w-6 p-0">
                      {copied ? <CheckCircle className="w-4 h-4 sm:w-4 sm:h-4 text-success" /> : <Copy className="w-4 h-4 sm:w-4 sm:h-4 text-muted-foreground" />}
                    </Button>
                  </div>
                ...
                scale: 1
                    ...
                    <div className="p-4 sm:p-6 rounded-2xl relative bg-white border-2 border-primary-light">
                      <QRCodeSVG value={account} size={window.innerWidth < 640 ? 140 : 180} bgColor="#FFFFFF" fgColor="#8B46FF" level="H" includeMargin={false} className="rounded-xl relative z-10" />
                  ...
                  repeat: Infinity
                      ...
                      <div className="flex items-center justify-center gap-2 sm:gap-2">
                        <img src={processedCoinImage || camlyCoinPro} alt="CAMLY Coin" className="w-10 h-10 sm:w-12 sm:h-12 object-contain" style={{
                      ...
                      animation: 'pulse 2s ease-in-out infinite'
                        ...
                        {tokens.find(t => t.symbol === "CAMLY")?.verified && <Shield className="w-4 h-4 sm:w-5 sm:h-5 text-success" />}
                      </div>
          ...
          delay: 0.1
              ...
              <div className="flex gap-2 sm:gap-3 overflow-x-auto pb-3 sm:pb-4 px-1 sm:px-2 scrollbar-hide snap-x snap-mandatory">
                {tokens.map(token => <motion.div key={token.symbol} whileHover={{
              scale: 1.05
            }} whileTap={{
              scale: 0.95
            }} className={`flex-shrink-0 rounded-xl sm:rounded-2xl transition-all duration-300 snap-center min-w-[100px] sm:min-w-[120px] relative ${selectedChartCoin === token.symbol ? 'p-1 bg-gradient-to-br from-purple-500 via-blue-500 to-purple-600 shadow-[var(--shadow-button)]' : selectedToken.symbol === token.symbol ? 'p-1 bg-gradient-to-br from-purple-400 via-blue-400 to-purple-500' : 'p-1 bg-gradient-to-br from-purple-300 via-blue-300 to-purple-400 opacity-70'}`}>
                    <motion.button onClick={() => {
                setSelectedToken(token);
                handleCoinChartToggle(token.symbol);
              }} className={`w-full h-full px-4 sm:px-6 py-3 sm:py-4 rounded-lg sm:rounded-xl font-bold transition-all duration-300 ${selectedChartCoin === token.symbol ? 'bg-gradient-to-br from-primary to-secondary text-white scale-105 ring-4 ring-primary/30' : selectedToken.symbol === token.symbol ? 'bg-gradient-to-br from-primary/20 to-secondary/20 text-foreground' : 'bg-card/80 text-foreground'}`}>
                    {selectedChartCoin === token.symbol && <motion.div initial={{
                  scale: 0
                }} animate={{
                  scale: 1
                }} className="absolute -top-1 -right-1 w-3 h-3 bg-green-500 rounded-full border-2 border-white" />}
                    <motion.div animate={selectedChartCoin === token.symbol ? {
                  rotate: 360
                } : {}} transition={{
                  duration: 2,
                  repeat: Infinity,
                  ease: "linear"
                      ...
                      {token.image ? <img src={token.symbol === "CAMLY" && processedCoinImage ? processedCoinImage : token.image} alt={token.symbol} className="w-10 h-10 sm:w-12 sm:h-12 object-contain" style={{
                    ...
                    {priceChanges[token.symbol] && Math.abs(priceChanges[token.symbol]) > 2 && <span className={`text-[9px] sm:text-[10px] font-bold mt-0.5 ${priceChanges[token.symbol] > 0 ? 'text-green-400' : 'text-red-400'}`}>
            ...
            {/* Price Chart Section */}
            <AnimatePresence>
              {selectedChartCoin && <motion.div initial={{
            height: 0,
            opacity: 0
          }} animate={{
            height: "auto",
                  ...
                  <Card className="gradient-border rounded-2xl bg-white backdrop-blur-sm overflow-hidden shadow-2xl">
                    <CardContent className="p-4 sm:p-6">
                      ...
                      ease: "linear"
                            ...
                            {tokens.find(t => t.symbol === selectedChartCoin)?.image ? <img src={selectedChartCoin === "CAMLY" && processedCoinImage ? processedCoinImage : tokens.find(t => t.symbol === selectedChartCoin)?.image} alt={selectedChartCoin} className="w-full h-full object-contain" style={{
                        ...
                        <div className="flex gap-1 sm:gap-2 bg-background/50 rounded-lg p-1">
                          {(['1H', '4H', '1D', '1W', '1M'] as const).map(tf => <motion.button key={tf} onClick={() => setChartTimeframe(tf)} whileHover={{
                      scale: 1.05
                    }} whileTap={{
                      scale: 0.95
                    }} className={`px-2 sm:px-3 py-1 sm:py-1.5 rounded text-[10px] sm:text-xs font-bold transition-all ${chartTimeframe === tf ? 'bg-primary text-white shadow-md' : 'text-muted-foreground hover:text-foreground'}`}>
                              {tf}
                            </motion.button>)}
                        </div>
                      </div>
                      ...
                      {/* Chart */}
                      {chartData.length > 0 ? <motion.div initial={{
                  opacity: 0
                }} animate={{
                  opacity: 1
                }} transition={{
                  delay: 0.2
                          ...
                          <ResponsiveContainer width="100%" height="100%">
                            <LineChart data={chartData}>
                              ...
                              <YAxis stroke="hsl(var(--muted-foreground))" fontSize={10} tickFormatter={value => `$${value.toFixed(selectedChartCoin === 'CAMLY' ? 6 : 2)}`} width={60} />
                        ...
                        color: 'hsl(var(--primary))'
                              ...
                              <Line type="monotone" dataKey="price" stroke="hsl(var(--primary))" strokeWidth={3} fill="url(#chartGradient)" dot={false} activeDot={{
                        r: 6,
                        ...
                        </motion.div> : <div className="h-48 sm:h-64 flex items-center justify-center">
                ...
                <CardContent className="p-4 sm:p-6">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    ease: "linear"
                        ...
                        {selectedToken.image ? <img src={selectedToken.symbol === "CAMLY" && processedCoinImage ? processedCoinImage : selectedToken.image} alt={selectedToken.symbol} className="w-full h-full object-contain" style={{
                    ...
                    repeat: Infinity
                      ...
                      <div className="flex items-center justify-end gap-2 mt-1">
                        ...
                        {priceChanges[selectedToken.symbol] && Math.abs(priceChanges[selectedToken.symbol]) > 2 && <span className={`text-xs font-bold flex items-center gap-0.5 ${priceChanges[selectedToken.symbol] > 0 ? 'text-green-500' : 'text-red-500'}`}>
          ...
          delay: 0.2
              ...
              <Tabs defaultValue="send" className="w-full">
                <TabsList className="grid w-full grid-cols-3 mb-4 sm:mb-6 p-1.5 sm:p-2 bg-muted rounded-xl sm:rounded-2xl">
                  <TabsTrigger value="send" className="font-black text-[11px] sm:text-base md:text-lg data-[state=active]:bg-gradient-to-r data-[state=active]:from-primary data-[state=active]:to-secondary data-[state=active]:text-white rounded-lg sm:rounded-xl py-2.5 sm:py-2">
                    <Send className="w-3.5 h-3.5 sm:w-5 sm:h-5 mr-1 sm:mr-2" />
                    <span className="hidden sm:inline">Normal Send</span>
                    <span className="sm:hidden">Send</span>
                  </TabsTrigger>
                  <TabsTrigger value="bulk" className="font-black text-[11px] sm:text-base md:text-lg data-[state=active]:bg-gradient-to-r data-[state=active]:from-primary data-[state=active]:to-secondary data-[state=active]:text-white rounded-lg sm:rounded-xl py-2.5 sm:py-2">
                    <Zap className="w-3.5 h-3.5 sm:w-5 sm:h-5 mr-1 sm:mr-2" />
                    <span className="hidden sm:inline">Bulk Airdrop</span>
                    <span className="sm:hidden">Bulk</span>
                  </TabsTrigger>
                  <TabsTrigger value="history" className="font-black text-[11px] sm:text-base md:text-lg data-[state=active]:bg-gradient-to-r data-[state=active]:from-primary data-[state=active]:to-secondary data-[state=active]:text-white rounded-lg sm:rounded-xl py-2.5 sm:py-2">
                ...
                <TabsContent value="send">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                    {/* P2P Transfer Card */}
                    {account && user && <P2PTransfer account={account} userId={user.id} camlyBalance={camlyBalance} onTransferComplete={async () => {
                  await getCamlyBalance(account);
                  await fetchTransactionHistory();
                }} />}
                    ...
                    <Card className="gradient-border rounded-2xl bg-white backdrop-blur-sm shadow-2xl relative overflow-hidden">
                      ...
                      boxShadow: '0 4px 20px rgba(139,70,255,0.3)'
                          ...
                          <motion.p className="font-mono text-base sm:text-lg md:text-xl text-foreground break-all mb-5 sm:mb-6 leading-relaxed font-black" animate={{
                        opacity: [0.8, 1, 0.8]
                      }} transition={{
                        duration: 2,
                        repeat: Infinity
                        ...
                        scale: 0.95
                            ...
                            <Button onClick={copyAddress} className="w-full font-black text-xl sm:text-2xl py-7 h-auto" size="lg" variant="secondary">
                      ...
                      }} variant="outline" className="text-sm sm:text-base h-10 sm:h-12 font-black px-6">
                            Test Feature ✨
                          </Button>
                        </div>
                      </CardContent>
                  ...
                  <Card className="gradient-border rounded-2xl bg-white backdrop-blur-sm shadow-2xl relative overflow-hidden">
                    ...
                    <CardContent className="space-y-4 sm:space-y-6">
                      <div>
                        ...
                        <Textarea value={bulkAddresses} onChange={e => {
                      setBulkAddresses(e.target.value);
                      setShowValidation(false); // Reset validation on change
                      setNeedsApproval(false);
                      setApprovalComplete(false);
                    }} onFocus={() => console.log("🔵 Focus bulk addresses textarea")} placeholder={`0x1234...\n0x5678...\n0x9abc...`} rows={6} className="border-0 text-foreground placeholder:text-foreground/30 font-mono text-xs sm:text-sm px-4 sm:px-3 py-3 sm:py-2 touch-manipulation bg-white" style={{
                      backdropFilter: 'blur(10px)',
                      boxShadow: 'inset 0 0 0 2px rgba(224,170,255,0.3)'
                    }} />
                        
                        {/* Validate Button */}
                        <Button onClick={validateAddresses} disabled={!bulkAddresses.trim()} className="mt-3 font-bold w-full sm:w-auto text-base sm:text-sm" variant="secondary">
                          <CheckCircle className="w-4 h-4 mr-2" />
                          VALIDATE ADDRESSES
                        </Button>
                        
                        {/* Validation Results */}
                        {showValidation && <motion.div initial={{
                      scale: 0.95,
                      opacity: 0
                    }} animate={{
                      scale: 1,
                      ...
                      background: validAddresses.length > 0 && invalidAddresses.length === 0 ? 'rgba(0,255,0,0.1)' : 'rgba(255,0,0,0.1)',
                      border: validAddresses.length > 0 && invalidAddresses.length === 0 ? '2px solid rgba(0,255,0,0.5)' : '2px solid rgba(255,0,0,0.5)'
                    }}>
                            <div className="space-y-2">
                              {validAddresses.length > 0 && <p className="text-green-400 font-bold">
                                  ✅ Valid: {validAddresses.length} address(es)
                                </p>}
                              {invalidAddresses.length > 0 && <div>
                                  <p className="text-red-400 font-bold mb-2">
                                    ❌ Invalid: {invalidAddresses.length} address(es)
                                  </p>
                                  <div className="text-xs text-red-300 max-h-32 overflow-y-auto space-y-1">
                                    {invalidAddresses.map((addr, idx) => <div key={idx} className="font-mono bg-red-900/20 p-1 rounded">
                                        {addr || "(empty line)"}
                                      </div>)}
                                  </div>
                                </div>}
                      ...
                      <div>
                        ...
                        <Input type="number" value={bulkAmount} onChange={e => {
                      setBulkAmount(e.target.value);
                      setNeedsApproval(false);
                      setApprovalComplete(false);
                    }} placeholder="1000" className="border-0 text-foreground text-xl sm:text-2xl font-black placeholder:text-foreground/30 h-14 sm:h-12 px-4 sm:px-3 bg-white" style={{
                      backdropFilter: 'blur(10px)',
                      boxShadow: 'inset 0 0 0 2px rgba(224,170,255,0.3)'
                    }} />
                      </div>

                      {bulkAddresses && bulkAmount && <motion.div initial={{
                    scale: 0.9,
                    opacity: 0
                  }} animate={{
                    scale: 1,
                    opacity: 1
                  }} className="p-4 sm:p-6 rounded-xl sm:rounded-2xl" style={{
                    ...
                    boxShadow: '0 0 40px rgba(255,215,0,0.3)'
                          ...
                          <div className="space-y-1.5 sm:space-y-2 text-foreground text-sm sm:text-base">
                            <p>🎯 Recipients: <span className="font-black text-foreground">
                              {showValidation && validAddresses.length > 0 ? `${validAddresses.length} (validated)` : bulkAddresses.split('\n').filter(a => a.trim()).length}
                            </span></p>
                            ...
                            <p className="text-xl sm:text-2xl">🚀 Total needed: <span className="font-black text-foreground">
                              {showValidation && validAddresses.length > 0 ? `${validAddresses.length} × ${bulkAmount} CAMLY` : `${bulkAddresses.split('\n').filter(a => a.trim()).length} × ${bulkAmount} CAMLY`}
                            </span></p>
                          </div>
                        </motion.div>}

                      {bulkSending && bulkProgress > 0 && <motion.div initial={{
                    scale: 0.9,
                    opacity: 0
                  }} animate={{
                    scale: 1,
                    opacity: 1
                      ...
                      {/* Approve Button */}
                      {showValidation && validAddresses.length > 0 && needsApproval && <motion.div initial={{
                    scale: 0.9,
                    opacity: 0
                  }} animate={{
                    scale: 1,
                    opacity: 1
                          ...
                          <Button onClick={handleApprove} disabled={approving} className="w-full font-black text-lg sm:text-xl md:text-2xl py-6 sm:py-7 mb-4 h-auto" size="lg">
                            {approving ? <>
                                <motion.div animate={{
                          rotate: 360
                        }} transition={{
                      ...
                      {showValidation && approvalComplete && !needsApproval && <motion.div initial={{
                    scale: 0.9,
                    opacity: 0
                  }} animate={{
                    scale: 1,
                    opacity: 1
                  }} className="p-3 sm:p-4 rounded-xl mb-3 sm:mb-4" style={{
                    ...
                    boxShadow: '0 0 30px rgba(0,255,0,0.4)'
                      ...
                      <Button onClick={handleBulkSendClick} disabled={bulkSending || !bulkAddresses || !bulkAmount || showValidation && validAddresses.length === 0 || showValidation && needsApproval} className="w-full font-black text-lg sm:text-xl md:text-2xl py-6 sm:py-7 h-auto" size="lg">
                        {bulkSending ? <>
                            <motion.div animate={{
                        rotate: 360
                      }} transition={{
      ...
      <AirdropConfirmModal isOpen={showConfirmModal} onClose={() => setShowConfirmModal(false)} onConfirm={handleBulkSend} walletCount={bulkAddresses.split('\n').filter(a => a.trim()).length} amountPerWallet={bulkAmount} totalAmount={parseFloat(bulkAmount) * bulkAddresses.split('\n').filter(a => a.trim()).length} estimatedGas={estimatedGas} gasPrice={currentGasPrice} />
